<?php
    ...
    class UserModel {
        ...
        public function validForm() {
            $existingUser = $this->_db->query("SELECT * FROM `users` WHERE `email` = '$this->email'");
            $existingUser = $existingUser->fetch(PDO::FETCH_ASSOC);

            if ($existingUser) {
                return "Користувач з таким електронним листом вже існує";
            }

            if (strlen($this->name) < 3)
                return "Логін занадто короткий";
            else if (strlen($this->email) < 3)
                return "Електронна пошта занадто коротка";
            else if (strlen($this->pass) < 3)
                return "Пароль має бути не менше 3 символів";
